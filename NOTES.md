...
- Commit `d8ed6af` replaces the bound-parameter `OR` cursor with independently indexed same-version and newer-version queries, preserving keyset order and the existing batch limit. It selects only fields used by municipal assignment, excluding the bulky `raw_coordinates` value, and aligns the recent-ingestion status order with the existing `(job_name, started_at DESC, id DESC)` index. It deployed as Worker version `b218a776-8312-4b05-9eed-c68350538b5e` without a container rollout or D1 migration. Post-deploy homepage, Hydro API, ingestion-health, and Archive-summary probes returned `200`; ingestion was seven minutes fresh with zero consecutive failures, and the expected cursor lag remained a warning. Production closure still requires a full rolling-day Query Insights comparison; no retained data was deleted.
...
- GitHub's hourly probe at `:17` subsequently produced false alarms: a successful `:07` ingestion could advance the municipal archive cursor before the bounded summary refresh, so fresh snapshots with zero failed runs returned `503` only for a cursor mismatch. The health contract now reports that expected lag in `warnings`; missing or internally contradictory summaries remain unhealthy. The public Archive payload still marks the lagging summary stale.
- Two consecutive Browser regression runs failed the mobile sheet-fetch recovery test before its server-rendered control had a reliable signal that JavaScript handlers were attached. The app now exposes `body[data-app-ready="true"]` immediately after `initSheet()`, and the test waits for it before intercepting and clicking.
...
- Release deployment: tagged commit `f6df621` deployed as Worker version `e6fe9a87-df8f-4cf4-a82b-b0dcdc07fa4c` with the `pannes-historiques-pannescontainer:e6fe9a87` image. Public `/`, Archive sheet, `/autocomplete?q=x&lang=en`, and `/api/health/ingestion` probes returned `200`. The health payload was healthy with zero consecutive failures and no archive-summary problem; the public materialized summary was served by `worker-d1`.
...
- The zone rule `autocomplete per IP` was installed through the authenticated dashboard on 2026-08-14. It is active for `GET /autocomplete`, counts by IP, and blocks after 10 matching requests in 10 seconds for 10 seconds. This is the only period and mitigation duration available to the Free plan. A controlled empty-query probe returned ten `200` responses, then `429` responses, then `200` after the 10-second window. Browser behavior for `429` is covered by Playwright; follow `docs/operations.md` for the repeatable procedure.
...
- Release commit `2fc094e` passed GitHub Quality run `29104581707` and was deployed on 2026-07-10 as Worker version `da3a0c51-d973-49b9-a9c0-9a2b819dd7e6` with container image `pannes-historiques-pannescontainer:da3a0c51`.
