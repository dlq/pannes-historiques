...
**Status:** Completed in `v0.2.7` and followed by cursor hardening at `9875b1a`. `origin/main` is now `c7fe3cb`, which also includes the merged `codex/frontend-stability-summary` slice. Production includes that frontend slice, and the service worker currently advertises `pannes-historiques-v0.2.7-outage-pin-icon`. Remaining work is operational maintenance/import/backfill hardening, not initial implementation.
