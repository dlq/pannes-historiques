...
It also returns `503` when the materialized archive summary is missing for a non-empty archive or contradicts itself. A source-cursor mismatch during the bounded refresh interval appears in `warnings`, while the public Archive response marks that stored summary stale; it does not make otherwise fresh ingestion unhealthy. Public Archive requests must not rebuild the summary from the large archive-bin tables. Coherence checks cover a shorter window exceeding the longer one containing it, a territory holding more outages than the whole year, or a largest single outage above the year's cumulative total. Those checks run against the summary actually being served, not a freshly built one. A problem here is a data-plane fault: ingestion may be perfectly current while the Archive report is missing or shows impossible figures. Read both `problems` and `warnings` to distinguish those conditions.
...
The hourly Worker maintenance schedule expires `ingestion_runs` left in `running` for more than three hours and purges terminal run records older than 30 days. It also prunes old derived current/planned marker rows after they have been superseded by a newer source version; raw Hydro API payloads remain in R2 and `hydro_snapshots` keeps their provenance. It checks municipal archive backfill progress hourly. Usage-evidence and geocode-cache retention cleanup are gated to run at most once per day. Maintenance does not delete raw R2 inputs, D1 geometry, municipal archive bins, or resolved-event history. See [ADR 0005](adr/0005-d1-archive-retention-and-compaction.md).
