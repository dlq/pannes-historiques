...
- Latest production deployment: post-release `main` commit `4fd7f94`, deployed 2026-09-06 as Worker version `fc638e25-b0c3-4ee3-bbb4-f12833ad7095`; the existing container image was unchanged. D1 migrations `0011` through `0013`, the 15-minute Hydro schedule, legacy-cron compatibility, live ingestion recovery, and the archive cursor warning/health split are production-verified.
...
- Production cost/health verification is substantially complete. The rolling 24-hour D1 totals on 2026-09-11 were `37,035,178` rows read and `396,087` rows written; a simple 30-day projection remains comfortably inside the Workers Paid included allowances. Ingestion was fresh, recent GitHub monitors passed, and repeated public map-context and Archive reads served unchanged stored materializations rather than request-time rebuilds. The archive cursor query, which read about `24.0 million` rows across 83 runs despite returning bounded batches, is now split locally into independently indexed same-version and newer-version ranges; it also omits unused raw coordinates. Deploy it, then compare the new query signatures and total reads over a full 24-hour window before considering the hotspot closed. Account-wide Container, Durable Objects, and R2 totals still require the authenticated dashboard review.
