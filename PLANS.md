...
- Latest production deployment: post-release `main` commit `83a6465`, deployed 2026-09-06 as Worker version `b0f70c69-43ba-49c3-abb2-3f9a2ca7a9b9`; the existing container image was unchanged. D1 migrations `0011` through `0013`, the 15-minute Hydro schedule, legacy-cron compatibility, and live ingestion recovery are production-verified.
...
- Cursor-aware materialized archive reads and ingestion-health checks are implemented. A source-cursor mismatch makes the served state visibly stale and appears as a health warning; a missing or internally contradictory summary makes the health probe unhealthy. Public requests must not rebuild the half-million-row archive summary. Focused tests cover matching, mismatch, missing cursors, incoherence, and the no-on-demand-rebuild guard. Verify the warning/health split after the next deployment.
...
- Pending production verification: confirm D1 row reads stay comfortably below the Free daily cap across a full UTC day after the one-time `0013` migration usage ages out, ingestion remains fresh, public map-context requests serve the stored summary, and no public Archive request triggers a summary rebuild.
...
- D1 measured `1,779,601,408` bytes (about `1.78 GB`) after migration `0013` on 2026-09-06, down about `550 MB` from the pre-migration `2.33 GB`. The database remains above the Free plan's per-database size allowance, so reducing or offloading bulky derived geometry and archive-bin materializations remains open even though raw R2 payloads, geometry provenance, archive-visible information, and snapshot metadata remain out of scope for automatic deletion. See ADR 0005.
