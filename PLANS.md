...
- Current cost posture: hybrid Worker/D1/R2 reads and the low-cost mode guardrail limit container exposure. Normal Hydro ingestion is ready to follow Hydro-Quebec's documented 15-minute source cadence once the `0013` indexes are applied, while cleanup and alerting run on a separate hourly maintenance cron with daily gates for retention work. D1 now treats old current/planned marker rows as reconstructable derived data: raw Hydro API payloads stay in R2, `hydro_snapshots` keeps provenance, and old derived marker rows are pruned before the `0013` indexes are added. The Archive summary and map-context payload are treated as bounded materializations, not routine request-path relational rebuilds: public reads serve stored summaries, stale Archive summaries are marked stale instead of rebuilt on demand, routine cost/status output avoids full D1 table-count scans, and migration `0013_d1_row_read_indexes.sql` targets the D1 indexes and keyed runtime summaries needed by the remaining freshness, ingestion-run, current-feed, disclosure-queue, map-context, archive-cursor, and archive-summary access patterns.
...
- Pending production verification: after deploying the 15-minute schedule and `0013` indexes, confirm D1 row reads stay comfortably below the Free daily cap across a full UTC day, ingestion remains fresh, public map-context requests serve the stored summary, and no public Archive request triggers a summary rebuild.
