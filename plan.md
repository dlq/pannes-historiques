...
- durable ingestion checkpoint as of 2026-05-05:
  ...
  - next migration step is to finish the current DAI R2 catch-up, then evaluate moving production disclosure/regional context reads from container SQLite to D1
  - DAI chunking is implemented: the container can collect selected disclosure source keys and export only those rows; the Worker selects due D1 sources one at a time, calls the container batch endpoint, and mirrors each completed source into D1/R2 within a bounded scheduled-run budget
  ...
  - per-source archival attempt/defer tracking is being deployed in D1: failed or slow sources are recorded, deferred for later, and no longer block other due DAI sources from being archived
  - DAI R2/D1 base catch-up completed on 2026-05-06 for all currently reachable sources: D1 reports `due_now = 0`, `29/32` disclosure sources have R2 keys, and the remaining 3 (`DAI-2022-0386`, `DAI-2025-0275`, `DAI-2025-0333`) are deferred with timeout errors until 2026-05-07
