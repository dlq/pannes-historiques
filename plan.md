...
- durable ingestion checkpoint as of 2026-05-05:
  ...
  - direct Worker-origin fetches to Hydro are currently receiving HTTP 406 while container-origin Hydro fetches succeed; keep Hydro feed collection resilient by continuing independent cron steps and prefer container-local payloads when archiving DAI source files to R2
  ...
  - next migration step is to verify the second D1 disclosure bootstrap after the internal source-file archival fix, verify raw DAI files in R2, then evaluate moving production disclosure/regional context reads from container SQLite to D1
