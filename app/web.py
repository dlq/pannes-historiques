...
from .views import render_page, render_result
...
function attachAddressAutocomplete() {
  ...
  const renderSuggestions = (items) => {
    ...
    items.forEach((item) => {
      ...
      button.type = "button";
      button.className = "block w-full rounded-xl px-3 py-2 text-left text-sm text-slate-700 hover:bg-stone-100";
...
class OutageTimeline extends HTMLElement {
  connectedCallback() {
    ...
    if (!items.length) {
      this.innerHTML = `<div class="rounded-2xl bg-stone-100 p-5 text-sm text-slate-600">No archived outage has been observed near this address yet.</div>`;
      return;
    }
    const bars = items.slice(0, 18).map((item) => {
...
class OutageMap extends HTMLElement {
  connectedCallback() {
    ...
    const bounds = [];
    if (data.center) {
      L.marker(data.center).addTo(map).bindPopup(data.addressLabel || "Address");
    ...
    (data.matches || []).forEach((item) => {
      ...
      if (!rendered && item.lat != null && item.lon != null) {
        L.circleMarker([item.lat, item.lon], {
          ...
          fillColor: color,
          fillOpacity: 0.65
        }).addTo(map).bindPopup(`${item.kind}: ${item.label || ""}`);
...
def create_app(settings: Settings):
    ...
    def application(environ, start_response):
        ...
        if path == "/static/app.js":
            ...
            return [STATIC_JS.encode("utf-8")]

        if path == "/collect" and method in {"POST", "GET"}:
            payload = service.collect()
            start_response("200 OK", [("Content-Type", "application/json; charset=utf-8")])
            return [json.dumps(serialize_payload(payload), ensure_ascii=True, indent=2).encode("utf-8")]

        if path == "/autocomplete" and method == "GET":
            params = parse_qs(environ.get("QUERY_STRING", ""))
            lang = choose_language(first(params, "lang"))
            query = first(params, "q", "")
            suggestions = service.geocoder.suggest(query, language=lang, limit=6)
            body = json.dumps({"suggestions": suggestions}, ensure_ascii=True)
            start_response("200 OK", [("Content-Type", "application/json; charset=utf-8")])
            return [body.encode("utf-8")]

        if path == "/search" and method == "POST":
            form = parse_form(environ)
            lang = choose_language(form.get("lang"))
            result = service.search(
                query=form.get("q", ""),
                language=lang,
                radius_m=int(form.get("radius_m") or settings.default_radius_m),
                days=int(form.get("days") or settings.default_days),
                include_planned=form.get("include_planned") == "1",
            )
            body = render_result(lang, result)
        ...
        if path == "/" and method == "GET":
            ...
            lang = choose_language(first(params, "lang"))
            query = first(params, "q", "")
            radius_m = int(first(params, "radius_m", settings.default_radius_m))
            days = int(first(params, "days", settings.default_days))
            include_planned = first(params, "include_planned", "1") == "1"
            result_html = ""
            if query:
                result = service.search(
                    query=query,
                ...
                result_html = render_result(lang, result)
            body = render_page(
                lang,
                settings,
                initial_query=query,
...
def first(params, key: str, default=None):
