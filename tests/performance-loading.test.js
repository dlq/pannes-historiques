...
test("service worker limits its cache to the offline navigation fallback", () => {
  ...
  assert.doesNotMatch(serviceWorker, /url\.pathname\.startsWith\("\/static\/"\)/);
  assert.doesNotMatch(serviceWorker, /cache\.put\(request, copy\)/);
});
